...
You can also select a color, which is then used for the trains in the navigator. If no color is selected, the default dark gray color is used or the color of the train category.

/// note
If a train is assigned to a train category that also has a color, then the color of the train line will still be used with priority. Only if the train line has no color then the color of the train category will be used instead.
///